...
Most_HR_Team=driver.find_element_by_css_selector('#_386451520446224440 > tbody:nth-child(34) > tr:nth-child(1) > td:nth-child(2) > a:nth-child(1)').text
Most_HR_Team
...
season_2014=driver.find_element_by_css_selector('#sp_hitting_season > option:nth-child(5)')
...
All_star=driver.find_element_by_css_selector('#sp_hitting_game_type > option:nth-child(2)')
...
data10_div=driver.find_element_by_id('datagrid')
data10_html=data10_div.get_attribute('innerHTML')
soup10=bs4.BeautifulSoup(data10_html,"html5lib")
head10=[t.text.replace("▼","") for t in soup10.thead.find_all("th")]
df_home10_run=pd.DataFrame(columns=head10)
df_home10_run
...
context_table_q11=[]
for t in soup10.tbody.find_all("tr"):
    for a in t.find_all("td"):
        context_table_q11.append(a.text)
context_table_q11


# In[196]:

context_table_q11_prettify=[]
for i in range(int(len(context_table_q11)/len(head10))):
    s=context_table_q11[i*len(head10):(i+1)*len(head10)]
    context_table_q11_prettify.append(s)
len(context_table_q11_prettify)
...
context_table_q11_prettify
for i in range(41):
    df_home10_run.loc[i]=context_table_q11_prettify[i]
    
df_home10_run.drop("",axis=1)


# In[198]:

df_home10_run.to_csv('Question_5.csv')


# In[237]:

latin_list='''Argentina;Bolivia;Brazil;Chile;Colombia;Costa Rica;Cuba;Dominican Republic;Ecuador;El Salvador;French Guiana;Guadeloupe;Guatemala;Haiti;Honduras;Martinique;Mexico;Nicaragua;Panama;Paraguay;Peru;Puerto Rico;Saint Barthélemy;Saint Martin;Uruguay;Venezuela'''


# In[238]:

latin_list=latin_list.split(';')
latin_list
...
player_list=[]
datan_div=driver.find_element_by_id('datagrid')
datan_html=datan_div.get_attribute('innerHTML')
soupn=bs4.BeautifulSoup(datan_html,"html5lib")
for t in soupn.tbody.find_all('tr'):
...
for name in player_list:
    player_link=driver.find_element_by_link_text(name)
    player_link.click()
    wait=WebDriverWait(driver, 10)
    p_bio=wait.until(EC.visibility_of_element_located((By.CLASS_NAME, 'player-bio')))
    for country in latin_list:
        if country in p_bio.text:
            player_fullname=driver.find_element_by_class_name('full-name').text
            time.sleep(20)
            teamname=wait.until(EC.visibility_of_element_located((By.CLASS_NAME, 'dropdown.team'))).text
            team=teamname.split('\n')[0].strip()
            print('full_name:', player_fullname )
            print('team:', team)
                      
#               print(name)
#               time.sleep(3)
#               player_info = wait.until(EC.visibility_of_element_located((By.CLASS_NAME, 'player-bio')))
    driver.back()
    time.sleep(20)
